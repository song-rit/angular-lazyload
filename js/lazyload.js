	app.controller(
			...
			function( $scope , ImageDataBase) {
				...
				$scope.photos = buildPhotoSet( 	ImageDataBase.getImageData().length );
				...
				function buildPhotoSet( size ) {
					...
					for ( var i = 0 ; i < size ; i++ ) {
						...
						photos.push({
							id: ( i + 1 ),
							// src: ( url + "christina-cox-" + index + ".jpg?v=" + version )
							  src: ( image[i].url + "?v" + version)
							  // src: ( "http://static.vecteezy.com/system/resources/previews/000/084/251/original/christmas-bokeh-vector-background.jpg"  )
						});

					}
					// photos = $scope.imageDataBase;
					// for( var i = 0; i < size; i++ ) {
					// 	photos.push()
					// }
